...
try:
    from .docker_utils import TestPostgreSQLConfig, docker_manager
except ImportError:
    ...
    from tests.lib.docker.docker_utils import TestPostgreSQLConfig, docker_manager
...
class TestComprehensivePostgreSQLAPI:
    ...
    def test_full_crud_operations_all_models(
        self: TestComprehensivePostgreSQLAPI, session: Any
    ) -> None:
        ...
        command = Command(
            ...
            owner="devops",
            runtime_env=json.dumps({"DATABASE_URL": "${DATABASE_URL}", "PYTHONPATH": "/app/src"}),
        )
        ...
        retrieved_goal = session.query(Goal).filter(Goal.title == "Comprehensive Test Goal").first()
        assert retrieved_goal is not None
        assert len(retrieved_goal.labels) == 3

        # Test JSON field queries
        complex_tasks = (
            session.query(Task).filter(Task.extra_data.contains("complexity_score")).all()
        )
    ...
    def test_complex_relationship_patterns(
        self: TestComprehensivePostgreSQLAPI, session: Any
    ) -> None:
        ...
        session.add_all([main_goal, development_phase, auth_task, api_task, *auth_steps, *labels])
        session.commit()
        ...
        high_priority_backend = (
            session.query(Task)
            .join(Task.labels)
            .filter(Task.severity == "high", Label.name == "backend")
            .all()
        )
        assert len(high_priority_backend) == 2

        # Find security-related items across all models
        security_items = session.query(Label).filter(Label.name == "security").first()
        assert security_items is not None

        # Test JSON queries for hierarchical data
        auth_related_steps = (
            session.query(Step).filter(Step.extra_data.contains("parent_task_id")).all()
        )
        assert len(auth_related_steps) == 3

    def test_yaml_import_export_functionality(
        self: TestComprehensivePostgreSQLAPI, session: Any
    ) -> None:
...
"""
        ...
        for phase_data in yaml_data["phases"]:
            phase = Phase(
                ...
                extra_data=json.dumps({"duration_weeks": phase_data.get("duration_weeks")}),
            )
        ...
        payment_task = (
            session.query(Task).filter(Task.title == "Payment Gateway Integration").first()
        )
    ...
    def _export_database_to_yaml(
        self: TestComprehensivePostgreSQLAPI, session: Any
    ) -> dict[str, Any]:
        """Export all database data to YAML-compatible format."""
        export_data = {"goals": [], "phases": [], "tasks": [], "labels": [], "commands": []}
    ...
    def test_postgresql_advanced_features(
                ...
                extra_data=json.dumps(
                    {
                        "metadata": {
                            ...
                            "nested": {"level1": {"level2": {"value": i, "active": i % 2 == 0}}},
                        },
                        "performance_metrics": {
                            "estimated_hours": i * 2,
        ...
        active_goals = session.query(Goal).filter(Goal.extra_data.contains("active")).all()
        assert len(active_goals) == 50  # Half should be active

        # Query specific JSON paths (PostgreSQL-specific)
        batch_goals = session.query(Goal).filter(Goal.extra_data.like('%"batch_id"%')).all()
        assert len(batch_goals) == 100

        # Test complex queries with ordering and limits
        high_complexity_goals = (
            session.query(Goal)
            .filter(Goal.extra_data.contains("complexity_score"))
        ...
        goal_counts_by_owner = (
            session.query(Goal.owner, func.count(Goal.id).label("goal_count"))
            .group_by(Goal.owner)
            .all()
        )
    ...
    def test_database_performance_and_scalability(
        self: TestComprehensivePostgreSQLAPI, postgresql_url: str
    ) -> None:
        ...
        try:
            ...
            indexed_result = (
                perf_session.query(Goal).filter(Goal.title == "Performance Goal 2500").first()
            )
            ...
            for page in range(total_pages):
                ...
                page_results = (
                    perf_session.query(Goal).order_by(Goal.id).offset(offset).limit(page_size).all()
                )
    ...
    def test_data_integrity_and_validation(
        self: TestComprehensivePostgreSQLAPI, session: Any
    ) -> None:
        ...
        valid_json_data = {
            ...
            "config": {"enabled": True, "timeout": 30, "nested_config": {"deep": True}},
        }
        ...
        retrieved_json_goal = (
            session.query(Goal).filter(Goal.title == "JSON Validation Test").first()
        )
        ...
        large_json_data = {
            ...
            "large_text": "x" * 10000,
            "nested_structure": {
                f"level_{i}": {f"key_{j}": f"value_{i}_{j}" for j in range(10)} for i in range(20)
            },
        }
        ...
        retrieved_large_goal = session.query(Goal).filter(Goal.title == "Large JSON Test").first()
    ...
    def test_transaction_and_concurrency_handling(
        self: TestComprehensivePostgreSQLAPI, postgresql_url: str
    ) -> None:
        ...
        try:
            ...
            for i in range(1, 3):
                invisible_goal = (
                    sessions[i].query(Goal).filter(Goal.title == "Isolation Test").first()
                )
            ...
            rolled_back_goal = sessions[1].query(Goal).filter(Goal.title == "Rollback Test").first()
