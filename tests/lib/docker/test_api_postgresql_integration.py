...
class TestAPIPostgreSQLIntegration:
    ...
    def test_complex_relationships_and_queries(
        self: TestAPIPostgreSQLIntegration, session: Any
    ) -> None:
        ...
        goals = [
            ...
            Goal(
                title="Mobile App", description="Mobile application", severity="medium", owner="bob"
            ),
            Goal(
        ...
        high_priority_alice_goals = (
            session.query(Goal).filter(Goal.severity == "high", Goal.owner == "alice").all()
        )
        ...
        backend_tasks_with_complexity = (
            session.query(Task).filter(Task.extra_data.like("%complexity%")).all()
        )
        assert len(backend_tasks_with_complexity) == 1

        # Test JSON field queries (PostgreSQL-specific)
        payment_tasks = session.query(Task).filter(Task.extra_data.contains("gateway")).all()
        assert len(payment_tasks) == 1

    def test_yaml_integration_with_postgresql(
        self: TestAPIPostgreSQLIntegration, session: Any
    ) -> None:
...
"""
        ...
        dev_goal = session.query(Goal).filter(Goal.title == "Setup Development Environment").first()
    ...
    def test_database_performance_and_scaling(
        self: TestAPIPostgreSQLIntegration, postgresql_url: str
    ) -> None:
        ...
        try:
            ...
            indexed_query = (
                perf_session.query(Goal).filter(Goal.title == "Performance Test Goal 500").first()
            )
    ...
    def test_transaction_isolation_and_concurrency(
        self: TestAPIPostgreSQLIntegration, postgresql_url: str
    ) -> None:
        ...
        try:
            ...
            for session in sessions:
                visible_goal = session.query(Goal).filter(Goal.title == "Isolation Test").first()
