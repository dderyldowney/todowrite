...
# Scenarios
@scenario(
    '../features/tractor_can_data.feature',
    'Receiving <Data Type> Data'
)
...
# When Steps
@when(parsers.parse('the Tractor ECU broadcasts {data_type} (PGN {pgn:d}, SPN {spn:d}) with value {value:d}'))
...
# Then Steps
@then(parsers.parse('the AFS FastAPI system should receive and correctly parse the {data_type} value as {value:d}'))
def system_receives_and_parses_data(afs_fastapi_system, isobus_network, data_type, value):
    ...
    found_message = next((
        msg for msg in afs_fastapi_system["can_messages_received"]
        if msg.get("data_type") == data_type and msg.get("parsed_data", {}).get(data_type.lower().replace(" ", "_")) == value
    ), None)
