...
def main():
    ...
    try:
        ...
        result = subprocess.run(
            ["bash", "-c", "source ~/.env && echo $CONTEXT7_API_KEY"],
            capture_output=True,
            text=True,
        )
    ...
    try:
        ...
        if (
            "context7: (2 tools)" in result.stderr
            and "github-official: (40 tools)" in result.stderr
        ):
    ...
    print(
        "💡 Full suite: Context7 + Docker + GitHub + Git + Filesystem + PostgreSQL + SQLite + Hugging Face + Playwright"
    )
