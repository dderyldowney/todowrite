...
class MCPSecurityOptimizer:
    ...
    def _check_forbidden_patterns(
        self, config: dict[str, Any], results: list[SecurityCheckResult]
    ) -> None:
    ...
    def _check_encryption_settings(
        self, config: dict[str, Any], results: list[SecurityCheckResult]
    ) -> None:
    ...
    def optimize_performance(
        ...
        logger.info(
            f"Performance optimization analysis completed for {config_name}: {len(metrics)} metrics"
        )
        return metrics
    ...
    def save_reports(
        self, security_report: dict[str, Any], performance_report: dict[str, Any], output_dir: Path
    ) -> None:
        """
        ...
        security_file = (
            output_dir / f"security_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
    ...
    def analyze_all_configurations(self) -> None:
        ...
        logger.info(
            f"Security: {security_report['summary']['passed']}/{security_report['summary']['total_checks']} checks passed"
        )
        logger.info(
            f"Performance: {performance_report['summary']['good_metrics']}/{performance_report['summary']['total_metrics']} metrics optimal"
        )
