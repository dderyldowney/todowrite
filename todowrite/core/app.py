...
class ToDoWrite:
    ...
    def _validate_partial_node_data(self, node_data: dict[str, Any]) -> None:
        """Validates partial node data against the ToDoWrite schema, ensuring data integrity."""
        if ToDoWrite._SCHEMA is None:
            raise ValueError("Schema not loaded. Cannot validate node data.")

        # Check for null values in non-null fields
        non_null_fields = ["id", "layer", "title"]
        for field in non_null_fields:
            if field in node_data and node_data[field] is None:
                raise ValueError(f"Field '{field}' cannot be null")

        # Check for empty strings in required fields
        required_non_empty = ["title"]  # Fields that cannot be empty strings
        for field in required_non_empty:
            if field in node_data and node_data[field] == "":
                raise ValueError(f"Field '{field}' cannot be empty")

        # Validate specific field types and enums
        if "layer" in node_data:
            valid_layers = ToDoWrite._SCHEMA["properties"]["layer"]["enum"]
            if node_data["layer"] not in valid_layers:
                raise ValueError(
                    f"Invalid layer '{node_data['layer']}'. Must be one of: {valid_layers}"
                )

        if "status" in node_data:
            valid_statuses = ToDoWrite._SCHEMA["properties"]["status"]["enum"]
            if node_data["status"] not in valid_statuses:
                raise ValueError(
                    f"Invalid status '{node_data['status']}'. Must be one of: {valid_statuses}"
                )
        ...
        if "metadata" in node_data and isinstance(node_data["metadata"], dict):
            ...
            # Check severity enum if present
            if "severity" in metadata:
                valid_severities = ToDoWrite._SCHEMA["properties"]["metadata"][
                    "properties"
                ]["severity"]["enum"]
                if metadata["severity"] not in valid_severities:
                    raise ValueError(
                        f"Invalid severity '{metadata['severity']}'. Must be one of: {valid_severities}"
                    )

            # Check work_type enum if present
            if "work_type" in metadata:
                valid_work_types = ToDoWrite._SCHEMA["properties"]["metadata"][
                    "properties"
                ]["work_type"]["enum"]
                if metadata["work_type"] not in valid_work_types:
                    raise ValueError(
                        f"Invalid work_type '{metadata['work_type']}'. Must be one of: {valid_work_types}"
                    )

        # Validate command fields if present
        if "command" in node_data and isinstance(node_data["command"], dict):
            command = node_data["command"]
            layer = node_data.get("layer")

            # Check ac_ref pattern if present and we know it's a Command layer
            if "ac_ref" in command and layer == "Command" or "ac_ref" in command:
                import re

                pattern = ToDoWrite._SCHEMA["properties"]["command"]["properties"][
                    "ac_ref"
                ]["pattern"]
                if not re.match(pattern, command["ac_ref"]):
                    raise ValueError(
                        f"Invalid ac_ref format '{command['ac_ref']}'. Must match pattern: {pattern}"
                    )

        # Validate progress range if present
        ...
        # Validate basic types for other fields
        type_checks = {
            "description": str,
            "owner": str,
            "assignee": str,
        ...
        for field, expected_type in type_checks.items():
            if field in node_data and not isinstance(node_data[field], expected_type):
                raise ValueError(
                    f"Field '{field}' must be of type {expected_type.__name__}"
                )

        # Validate links structure if present
        if "links" in node_data and isinstance(node_data["links"], dict):
            links = node_data["links"]
            if not isinstance(links.get("parents"), list) or not all(
                isinstance(p, str) for p in links.get("parents", [])
            ):
                raise ValueError("links.parents must be a list of strings")
            if not isinstance(links.get("children"), list) or not all(
                isinstance(c, str) for c in links.get("children", [])
            ):
                raise ValueError("links.children must be a list of strings")

        # Validate labels if present
        if "labels" in node_data and isinstance(node_data["labels"], list):
            if not all(isinstance(label, str) for label in node_data["labels"]):
    ...
    def update_node(self, node_id: str, node_data: dict[str, Any]) -> Node | None:
        ...
        # Validate partial update to maintain data integrity
        self._validate_partial_node_data(node_data)
    ...
    def delete_node(self, node_id: str) -> None:
        ...
        with self.get_db_session() as session:
            ...
            db_node = session.execute(stmt).scalar_one_or_none()
            if db_node:
                # Handle Command layer deletion to avoid foreign key constraint violations
                if db_node.layer == "Command":
                    # Delete the command relationship first
                    if db_node.command:
                        # Delete artifacts first
                        for artifact in db_node.command.artifacts:
                            session.delete(artifact)
                        # Then delete the command
                        session.delete(db_node.command)
