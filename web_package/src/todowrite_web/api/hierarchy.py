...
async def move_hierarchy_item(
    operation: MoveOperation, db: Session = Depends(get_db)
) -> HierarchyResponse:
    """
    ...
    try:
        ...
        dragged_item = (
            db.query(dragged_model)
            .filter(dragged_model.id == operation.dragged_item_id)
            .first()
        )

        target_item = (
            db.query(target_model)
            .filter(target_model.id == operation.target_item_id)
            .first()
        )
        ...
        if operation.operation_type == "move_to_parent" and operation.new_parent_id:
            ...
            return await handle_cross_parent_move(
                operation, db, dragged_item, target_item
            )
        else:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
...
async def handle_move_to_parent(
    operation: MoveOperation, db: Session, dragged_item: Base
) -> HierarchyResponse:
    ...
    if not validate_parent_child(
        operation.new_parent_type, operation.dragged_item_type
    ):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
    ...
    new_parent = (
        ...
        .filter(parent_model.id == operation.new_parent_id)
        .first()
    )
    ...
    # Add to new parent
    association_table = get_association_table(
        operation.new_parent_type, operation.dragged_item_type
    )
    if association_table:
        ...
        db.execute(
            query,
            {
                ...
                "child_id": operation.dragged_item_id,
            },
        )
...
async def handle_reorder(
    operation: MoveOperation, db: Session, dragged_item: Base, target_item: Base
) -> HierarchyResponse:
    ...
    return HierarchyResponse(
        success=True, message=f"Successfully reordered {operation.dragged_item_type}"
    )
...
async def handle_cross_parent_move(
    ...
    association_table = get_association_table(
        operation.target_item_type, operation.dragged_item_type
    )
    if association_table:
        query = text(f"""
            INSERT INTO {association_table}
        ...
        db.execute(
            query,
            {
                "parent_id": operation.target_item_id,
                "child_id": operation.dragged_item_id,
            },
        )
...
async def get_hierarchy_tree(
    parent_type: str, parent_id: int, max_depth: int = 3, db: Session = Depends(get_db)
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        parent_item = (
            db.query(parent_model).filter(parent_model.id == parent_id).first()
        )
        ...
        return {"parent_type": parent_type, "parent_id": parent_id, "tree": tree}
...
async def validate_move(
    ...
    try:
        ...
        item_exists = (
            db.query(item_model).filter(item_model.id == item_id).first() is not None
        )
        parent_exists = (
            db.query(parent_model).filter(parent_model.id == parent_id).first()
            is not None
        ...
        if not item_exists:
            return {"valid": False, "reason": f"Item not found: {item_type} #{item_id}"}
        ...
        return {"valid": False, "reason": f"Validation error: {str(e)}", "warnings": []}
